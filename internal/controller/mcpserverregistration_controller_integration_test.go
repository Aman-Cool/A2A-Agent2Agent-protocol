...
var _ = Describe("MCPServerRegistration Controller", func() {
	...
	Context("prefix uniqueness validation", func() {
		const (
			gatewayName    = "prefix-test-gw"
			serviceName    = "prefix-test-svc"
			httpRouteName1 = "prefix-test-route1"
			httpRouteName2 = "prefix-test-route2"
		)
		...
		BeforeEach(func() {
			...
			httpRoute1 := createTestHTTPRoute(httpRouteName1, "default", "prefix-test1.mcp.local", serviceName, 8080, gatewayName, "default")
			Expect(testK8sClient.Create(ctx, httpRoute1)).To(Succeed())
			Expect(setHTTPRouteAcceptedStatus(ctx, httpRoute1, gatewayName, "default")).To(Succeed())

			// create second HTTPRoute
			httpRoute2 := createTestHTTPRoute(httpRouteName2, "default", "prefix-test2.mcp.local", serviceName, 8080, gatewayName, "default")
			Expect(testK8sClient.Create(ctx, httpRoute2)).To(Succeed())
			Expect(setHTTPRouteAcceptedStatus(ctx, httpRoute2, gatewayName, "default")).To(Succeed())
		})

		AfterEach(func() {
			forceDeleteTestMCPServerRegistration(ctx, "prefix-unique-1", "default")
			forceDeleteTestMCPServerRegistration(ctx, "prefix-unique-2", "default")
			forceDeleteTestMCPServerRegistration(ctx, "prefix-unique-3", "default")
			deleteTestHTTPRoute(ctx, httpRouteName1, "default")
			deleteTestHTTPRoute(ctx, httpRouteName2, "default")
			deleteTestService(ctx, serviceName, "default")
			deleteTestGateway(ctx, gatewayName, "default")
		})

		It("allows different prefixes on multiple registrations", func() {
			mcpsr1 := createTestMCPServerRegistration("prefix-unique-1", "default", httpRouteName1, "abc_")
			Expect(testK8sClient.Create(ctx, mcpsr1)).To(Succeed())

			mcpsr2 := createTestMCPServerRegistration("prefix-unique-2", "default", httpRouteName2, "def_")
			Expect(testK8sClient.Create(ctx, mcpsr2)).To(Succeed())

			// Both should reach Ready status
			Eventually(func(g Gomega) {
				retrieved1 := &mcpv1.MCPServerRegistration{}
				g.Expect(testK8sClient.Get(ctx, types.NamespacedName{Name: "prefix-unique-1", Namespace: "default"}, retrieved1)).To(Succeed())
				cond := meta.FindStatusCondition(retrieved1.Status.Conditions, "Ready")
				g.Expect(cond).NotTo(BeNil())
				g.Expect(cond.Status).To(Equal(metav1.ConditionTrue))
			}, testTimeout, testRetryInterval).Should(Succeed())

			Eventually(func(g Gomega) {
				retrieved2 := &mcpv1.MCPServerRegistration{}
				g.Expect(testK8sClient.Get(ctx, types.NamespacedName{Name: "prefix-unique-2", Namespace: "default"}, retrieved2)).To(Succeed())
				cond := meta.FindStatusCondition(retrieved2.Status.Conditions, "Ready")
				g.Expect(cond).NotTo(BeNil())
				g.Expect(cond.Status).To(Equal(metav1.ConditionTrue))
			}, testTimeout, testRetryInterval).Should(Succeed())
		})

		It("rejects duplicate prefix on create", func() {
			// Create first registration with prefix "xyz_"
			mcpsr1 := createTestMCPServerRegistration("prefix-unique-1", "default", httpRouteName1, "xyz_")
			Expect(testK8sClient.Create(ctx, mcpsr1)).To(Succeed())

			// Wait for first registration to be ready
			Eventually(func(g Gomega) {
				retrieved1 := &mcpv1.MCPServerRegistration{}
				g.Expect(testK8sClient.Get(ctx, types.NamespacedName{Name: "prefix-unique-1", Namespace: "default"}, retrieved1)).To(Succeed())
				cond := meta.FindStatusCondition(retrieved1.Status.Conditions, "Ready")
			...
			// Attempt to create second registration with same prefix
			mcpsr2 := createTestMCPServerRegistration("prefix-unique-2", "default", httpRouteName2, "xyz_")
			Expect(testK8sClient.Create(ctx, mcpsr2)).To(Succeed())

			// Second registration should have PrefixConflict condition
			Eventually(func(g Gomega) {
				retrieved2 := &mcpv1.MCPServerRegistration{}
				g.Expect(testK8sClient.Get(ctx, types.NamespacedName{Name: "prefix-unique-2", Namespace: "default"}, retrieved2)).To(Succeed())
				cond := meta.FindStatusCondition(retrieved2.Status.Conditions, "Ready")
				g.Expect(cond).NotTo(BeNil())
				g.Expect(cond.Status).To(Equal(metav1.ConditionFalse))
				g.Expect(cond.Reason).To(Equal("PrefixConflict"))
				g.Expect(cond.Message).To(ContainSubstring("xyz_"))
				g.Expect(cond.Message).To(ContainSubstring("prefix-unique-1"))
			}, testTimeout, testRetryInterval).Should(Succeed())
		})

		It("allows multiple registrations with empty prefix", func() {
			mcpsr1 := createTestMCPServerRegistration("prefix-unique-1", "default", httpRouteName1, "")
			Expect(testK8sClient.Create(ctx, mcpsr1)).To(Succeed())

			mcpsr2 := createTestMCPServerRegistration("prefix-unique-2", "default", httpRouteName2, "")
			Expect(testK8sClient.Create(ctx, mcpsr2)).To(Succeed())

			// Both should reach Ready (empty prefix is allowed and doesn't conflict)
			Eventually(func(g Gomega) {
				retrieved1 := &mcpv1.MCPServerRegistration{}
				g.Expect(testK8sClient.Get(ctx, types.NamespacedName{Name: "prefix-unique-1", Namespace: "default"}, retrieved1)).To(Succeed())
				cond := meta.FindStatusCondition(retrieved1.Status.Conditions, "Ready")
		...
		It("allows substring prefixes that do not exactly match", func() {
			// Create registration with prefix "app_"
			mcpsr1 := createTestMCPServerRegistration("prefix-unique-1", "default", httpRouteName1, "app_")
			Expect(testK8sClient.Create(ctx, mcpsr1)).To(Succeed())
			...
			// Create registration with prefix "app_admin_" (substring of first, but not exact match)
			// This should be allowed per design (longest-prefix-match resolves routing)
			mcpsr2 := createTestMCPServerRegistration("prefix-unique-2", "default", httpRouteName2, "app_admin_")
			Expect(testK8sClient.Create(ctx, mcpsr2)).To(Succeed())

			// Second registration should also reach Ready
			Eventually(func(g Gomega) {
				retrieved2 := &mcpv1.MCPServerRegistration{}
				g.Expect(testK8sClient.Get(ctx, types.NamespacedName{Name: "prefix-unique-2", Namespace: "default"}, retrieved2)).To(Succeed())
				cond := meta.FindStatusCondition(retrieved2.Status.Conditions, "Ready")
		...
		It("recovers from conflict after conflicting registration is deleted", func() {
			// Create first registration
			mcpsr1 := createTestMCPServerRegistration("prefix-unique-1", "default", httpRouteName1, "conflict_")
			Expect(testK8sClient.Create(ctx, mcpsr1)).To(Succeed())
			...
			// Create second registration with conflicting prefix
			mcpsr2 := createTestMCPServerRegistration("prefix-unique-2", "default", httpRouteName2, "conflict_")
			Expect(testK8sClient.Create(ctx, mcpsr2)).To(Succeed())
			...
			// Second registration should eventually recover to Ready status
			Eventually(func(g Gomega) {
				retrieved2 := &mcpv1.MCPServerRegistration{}
				g.Expect(testK8sClient.Get(ctx, types.NamespacedName{Name: "prefix-unique-2", Namespace: "default"}, retrieved2)).To(Succeed())
				cond := meta.FindStatusCondition(retrieved2.Status.Conditions, "Ready")
